...
class GameWidget(Widget):
    def __init__(self, **kwargs):
        ...
        self._score_label = CoreLabel(text="Score: -", markup=True,
                                      font_size=36, color=[1, 1, 0, 1],
        ...
        with self.canvas:
            Rectangle(source="assets/bkg800-600.jpg", pos=(0, 0),
                      size=(Window.width, Window.height))
            self._score_instruction = Rectangle(texture=self._score_label.texture, pos=(
                20, Window.height - 40), size=self._score_label.texture.size)
...
class Bullet(Entity):
    def __init__(self, pos, speed=300):
        super().__init__()
        sound = SoundLoader.load("assets/laser_sound2.wav")
        sound.play()
        self._speed = speed
        self.pos = pos
...
class Explosion(Entity):
    def __init__(self, pos):
        super().__init__()
        self.pos = pos
        sound = SoundLoader.load("assets/kill_sound.wav")
        self.source = "assets/explosion2.png"
        sound.play()
